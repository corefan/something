...
if (!nexacro.FileUpload) {
	...
	_pFileUpload.on_load = function (status, data, url, errcode, httpcode, errmsg) {
		var code = errcode ? errcode : -1;
		var msg = errmsg ? errmsg : "";
		try {
			...
			if (fstr == "XML") {
				result = nexacro.TransactionItem.prototype.__deserializeXML.call(this, "", xmldoc);
			}
			else if (fstr == "SSV") {
				result = nexacro.TransactionItem.prototype.__deserializeSSV.call(this, data);
			}
			else {
				this.on_fire_onerror(this, "CommunicationError", data, this, 9902, null, null, -1);
				delete this.context;
				return;
			}
			delete this.context;
			...
			if (code < 0 || status < 0) {
				this.on_fire_onerror(this, "CommunicationError", msg, this, 9902, null, null, -1);
			}
			else {
				this.on_fire_onsuccess(result[3], code, msg, url);
			}
		}
		catch (e) {
			msg = "failed to get";
