...
(function(ap) {
    ...
    function q(aJ, aL, aQ, aN) {
        ...
        for (aM = 0; aM < aJ; ++aM) {
            ...
            j = F(1 - aP * aP);
            aO = aM * aK;
            aR.push([w(aO) * j * aL, aP * aQ, ah(aO) * j * aN])
        }
        return aR
    }

    function W(aK, aI, aN, aT, aR) {
        var aS, aU = [],
            aL = Math.PI * (3 - F(5)),
            aJ = 2 / aK,
            aQ, aP, aO, aM;
        for (aQ = 0; aQ < aK; ++aQ) {
            ...
            aO = w(aS);
            aM = ah(aS);
            aU.push(aI ? [aP * aN, aO * aT, aM * aR] : [aO * aN, aP * aT, aM * aR])
        }
        return aU
    }
    ...
    function am(aJ, i, j, aI) {
        return W(aJ, 0, i, j, aI)
    }

    function av(aJ, i, j, aI) {
        return W(aJ, 1, i, j, aI)
    }
            ...
            if (!aQ.HasText()) {
                ...
                if (aL.bgColour || aL.bgOutlineThickness) {
                    ...
                    if (aL.outlineMethod == "colour") {
                        aJ = aD(aQ.fimage, aK, aP, aO, aN, aL.bgOutlineThickness, aL.outlineColour, aL.padding, aL.bgRadius, 1);
                        if (aJ) {
                            aQ.oimage = aJ.image
                        }
                    }
    ...
    x.DrawSimple = function(aK, i, j, aJ) {
        var aI = this.tc;
    ...
    x.DrawPulsate = function(aL, i, j, aJ) {
        var aK = G() - this.ts,
            aI = this.tc;
        ...
        aL.globalAlpha = aI.pulsateTo + ((1 - aI.pulsateTo) * (0.5 + (w(2 * Math.PI * aK / (1000 * aI.pulsateTime)) / 2)));
        return this.drawFunc(aL, this.x, this.y, this.w, this.h, aI.outlineColour, i, j, aJ)
    };
    x.Active = function(aI, i, j) {
    ...
    c.Measure = function(aN, aQ) {
        ...
        if (aQ.txtOpt) {
            ...
            if (aQ.outlineMethod == "colour") {
                this.oimage = aL.Create(aQ.outlineColour, this.bgColour, aQ.outlineColour, aR * this.bgOutlineThickness, aQ.shadow, aR * aQ.shadowBlur, j, aR * this.padding, aR * this.bgRadius)
            } else {
    ...
    Q.Load = function() {
        var aS,aN, aQ, aR, aM, aJ, aI, j, aK, aP, aL, aO;
